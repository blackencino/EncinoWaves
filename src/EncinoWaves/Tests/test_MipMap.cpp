...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1) {
    params.resolutionPowerOfTwo = atoi(argv[1]);
  }
  ...
  float ftime = 96.0f / 24.0f;
  prop.propagate(params, istate, pstate, ftime);
  std::cout << "Propagated to frame: " << 96 << std::endl
  ...
  ewav::Statsf s1(pstate.Height, pstate.MinE);

  ewav::PropagatedStatef pstateDown(params.resolutionPowerOfTwo - 1);
  std::cout << "Created half-sized state." << std::endl;

  ewav::DownsampleState<float>(pstate, pstateDown);
  std::cout << "Downsampled half-sized state from state." << std::endl;

  ewav::Statsf s2(pstateDown.Height, pstateDown.MinE);
